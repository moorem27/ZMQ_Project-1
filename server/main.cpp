...
int main( void ) {
    ...
    server.sin_port = htons( 5555 );        // Use htons to convert from host byte order to network byte order
    ...
    // (In a separate terminal enter 'bmon' to monitor network traffic)
    std::cout << "Listener is waiting" << std::endl;

    address_length = sizeof( server );
    int count = 0;
    while( ++count < 1000 ) {
        bytes = recvfrom( socket_file_descriptor, buffer, sizeof( buffer ), 0, (struct sockaddr*)&server, &address_length );
        if( bytes < 0 ) {
